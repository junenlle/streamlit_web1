...
def page_eight():
    ...
    def process_transfer_file(file,start_date,end_date):
        ...
        # 按周分组数据，每周开始时间为星期五
        weekly_big_grouped = big_df.groupby(pd.Grouper(key='中转日期', freq='W-MON', label='left', closed='left'))
        ...
        # 按周分组数据，每周开始时间为星期五
        weekly_small_grouped = small_df.groupby(pd.Grouper(key='中转日期', freq='W-MON', label='left', closed='left'))
    ...
    def process_daily_resource_file(file,start_date,end_date):
        ...
        # 按周分组数据，每周开始时间为星期五
        weekly_grouped = region_df.groupby(pd.Grouper(key='计划发车日期', freq='W-MON', label='left', closed='left'))
    ...
    def process_pickup_file(file,start_date, end_date):
        ...
        # 按周分组数据，每周开始时间为星期五
        weekly_grouped = df.groupby(pd.Grouper(key='开始时间', freq='W-MON', label='left', closed='left'))
    ...
    def process_deliver_file(file,start_date, end_date):
        ...
        # 按周分组数据，每周开始时间为星期五
        weekly_grouped = df.groupby(pd.Grouper(key='日期', freq='W-MON', label='left', closed='left'))
