...
def page_ten():
    ...
    if uploaded_file is not None:
        ...
        if st.button('开始训练'):
            ...
            # 重新构建XGBoost模型
            model_pickups = xgb.XGBRegressor(objective='reg:squarederror', n_estimators=100, learning_rate=0.1, max_depth=5)
            model_delivers = xgb.XGBRegressor(objective='reg:squarederror', n_estimators=100, learning_rate=0.1, max_depth=5)

            # 训练模型并保存训练历史记录
            dtrain_pickups = xgb.DMatrix(X_train, label=y_train[:, 0])
            dval_pickups = xgb.DMatrix(X_val, label=y_val[:, 0])
            evals_pickups = [(dtrain_pickups, 'train'), (dval_pickups, 'val')]
            params_pickups = {'objective': 'reg:squarederror', 'learning_rate': 0.1, 'max_depth': 5}
            eval_results_pickups = {}
            model_pickups = xgb.train(params_pickups, dtrain_pickups, num_boost_round=100, evals=evals_pickups, evals_result=eval_results_pickups, verbose_eval=False)

            dtrain_delivers = xgb.DMatrix(X_train, label=y_train[:, 1])
            dval_delivers = xgb.DMatrix(X_val, label=y_val[:, 1])
            evals_delivers = [(dtrain_delivers, 'train'), (dval_delivers, 'val')]
            params_delivers = {'objective': 'reg:squarederror', 'learning_rate': 0.1, 'max_depth': 5}
            eval_results_delivers = {}
            model_delivers = xgb.train(params_delivers, dtrain_delivers, num_boost_round=100, evals=evals_delivers, evals_result=eval_results_delivers, verbose_eval=False)

            # 保存模型
            joblib.dump(model_pickups, 'model_pickups.pkl')
            joblib.dump(model_delivers, 'model_delivers.pkl')

            # 预测
            train_predict_pickups = model_pickups.predict(dtrain_pickups).reshape(-1, 1)
            test_predict_pickups = model_pickups.predict(xgb.DMatrix(X_test)).reshape(-1, 1)
            train_predict_delivers = model_delivers.predict(dtrain_delivers).reshape(-1, 1)
            test_predict_delivers = model_delivers.predict(xgb.DMatrix(X_test)).reshape(-1, 1)

            train_predict = np.concatenate((train_predict_pickups, train_predict_delivers), axis=1)
            test_predict = np.concatenate((test_predict_pickups, test_predict_delivers), axis=1)
            ...
            def dynamic_predict_future(model_pickups, model_delivers, df, time_step, num_days, scaler):
                # 初始化未来预测数据
                ...
                for i in range(num_days):
                    ...
                    # 进行预测
                    next_day_predict_pickups = model_pickups.predict(xgb.DMatrix(scaled_future_data))
                    next_day_predict_delivers = model_delivers.predict(xgb.DMatrix(scaled_future_data))
                    
                    # 反归一化
                    next_day_predict = np.array([next_day_predict_pickups[0], next_day_predict_delivers[0]])
                    next_day_predict = scaler.inverse_transform(np.concatenate((next_day_predict.reshape(1, 2), np.zeros((1, 4))), axis=1))[0][:2]
                    ...
                    new_row = np.concatenate((next_day_predict, [is_major_peak, is_minor_peak, is_holiday, is_workday]))
                    ...
                    new_data = {
                        'pickups': next_day_predict[0],
                        'delivers': next_day_predict[1],
                    ...
                    # 保存预测结果
                    future_predict.append(next_day_predict)
            ...
            future_predict, future_dates, df_extended = dynamic_predict_future(model_pickups, model_delivers, df, time_step, num_days, scaler)
            ...
            ax1.plot(eval_results_pickups['train']['rmse'], label='训练损失 (收件量)')
            ax1.plot(eval_results_pickups['val']['rmse'], label='验证损失 (收件量)', linestyle='--')
            ax1.plot(eval_results_delivers['train']['rmse'], label='训练损失 (派件量)', color='orange')
            ax1.plot(eval_results_delivers['val']['rmse'], label='验证损失 (派件量)', color='orange', linestyle='--')
            ...
            ax2[0].plot(df.index[time_step+1:train_size], train_predict_pickups, label='预测收件量', color='red')
            if test_predict_pickups.size > 0:
                ax2[0].plot(test_indices, y_test[:, 0], label='实际收件量 (测试集)')
                ax2[0].plot(test_indices, test_predict_pickups, label='预测收件量 (测试集)', color='orange')
            ...
            ax2[1].plot(df.index[time_step+1:train_size], train_predict_delivers, label='预测派件量', color='red')
            if test_predict_delivers.size > 0:
                ax2[1].plot(test_indices, y_test[:, 1], label='实际派件量 (测试集)')
                ax2[1].plot(test_indices, test_predict_delivers, label='预测派件量 (测试集)', color='orange')
